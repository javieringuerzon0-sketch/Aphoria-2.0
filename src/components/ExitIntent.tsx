...
const ExitIntent: React.FC = () => {
  ...
  // ── Scroll lock — same pattern as CartDrawer ──────────────────────
  useLayoutEffect(() => {
    const html = document.documentElement;
    if (show) {
      const scrollY = window.scrollY;
      html.style.setProperty('--popup-lock-y', `-${scrollY}px`);
      html.classList.add('popup-open');
    } else {
      const raw = html.style.getPropertyValue('--popup-lock-y');
      const savedY = raw ? Math.abs(parseInt(raw, 10)) : 0;
      html.classList.remove('popup-open');
      html.style.removeProperty('--popup-lock-y');
  ...
  return (
    <>
      {/* Overlay — always mounted, no backdrop-blur, exempt from transition-kill */}
      <motion.div
        data-exit-popup
        initial={false}
      ...
      >
        {/* Modal — always mounted, exempt from transition-kill */}
        <motion.div
          data-exit-popup
          initial={false}
          animate={{
